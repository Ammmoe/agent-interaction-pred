...
class DroneRelationModel(nn.Module):
    def __init__(self, context_dim, hidden_dim=None, device="cuda"):
        super().__init__()
        self.device = device
        if hidden_dim is None:
            hidden_dim = context_dim  # default to match embedding
        self.hidden_dim = hidden_dim
    ...
    def forward(self, current_features, context_embeddings, relationships):
        ...
        Args:
            current_features: [num_drones, 4]
            context_embeddings: [num_drones, hidden_dim]  # already decoder embeddings
            relationships: [num_pairs, 2] (pairs of drone indices)
        Returns:
            preds: [num_pairs]
        """
        # Use pretrained embeddings directly as node features
        node_emb = context_embeddings  # [num_drones, hidden_dim]
